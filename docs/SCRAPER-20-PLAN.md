# Sprint 20+ Plan — fuqing-scraper 治理 backlog
...
Sprint 16 Wave 1 (v0.4.14.39) 拆出独立 repo, Sprint 16.5+1 (v0.4.14.40) 5 文档同步, Sprint 19+ #141 (v0.4.14.41) 治根 check_dmp_session 业务层 session 验证, B1 治根 (v0.4.14.53) 跨子项目依赖解耦, GitHub 推送 weiweity/dmp-data-scraper main = 06cc0f3。 Sprint 20+ 5 工单留 backlog, 跟主项目 fuqing-crm-analytics 9bd4274 同步, 跑批业务不阻塞 (data3.csv 7164 → 7209, 0 行污染)。
...
- **步骤**:
  ...
  6. 保留 1.3 技术栈跟 3 三层架构跟 4 核心模块详解跟 6 数据策略跟 7 自动化机制跟 8 约束速查跟 8.6 历史事故跟 8.7 待办跟 8.8 反检测跟 8.9 错误排查 跟 Sprint 20+ 状态
- **风险**: 简历文档不在 git 里, 写完不 commit
- **执行**: Sprint 20+ 第 5 工单 (留 Sprint 21+)

## 执行顺序 (按依赖)

#19 (Sprint 20+ 计划) → #15 (主项目软删) → #16 (双层清理) → #17 (5 行修) → #18 (简历更新)
