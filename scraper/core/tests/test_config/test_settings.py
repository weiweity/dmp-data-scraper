...
from scraper.core.config.settings import Config, CONFIG_ITEMS_YAML
...
def test_config_load_items_reads_yaml(tmp_path: Path) -> None:
    ...
    with patch('scraper.core.config.settings.CONFIG_ITEMS_YAML', yaml_path):
...
def test_config_load_items_missing_file_raises(tmp_path: Path) -> None:
    ...
    with patch('scraper.core.config.settings.CONFIG_ITEMS_YAML', missing):
...
def test_config_load_items_invalid_schema_raises(tmp_path: Path) -> None:
    ...
    with patch('scraper.core.config.settings.CONFIG_ITEMS_YAML', bad_yaml):
    ...
    with patch('scraper.core.config.settings.CONFIG_ITEMS_YAML', bad_yaml2):
        with pytest.raises(ValueError, match="schema invalid"):
            Config.load_items()
...
def test_config_class_attributes_unchanged() -> None:
    ...
    assert Config._SCRIPT_DIR.endswith('/scraper/core')
